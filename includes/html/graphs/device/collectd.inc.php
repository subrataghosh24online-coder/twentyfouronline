...
function error($code, $code_msg, $title, $msg)
{
    ...
    $w = (twentyfouronlineConfig::get('rrd_width') + 81);
    $h = (twentyfouronlineConfig::get('rrd_height') + 79);
    ...
    if (function_exists('imagettfbbox') && is_file(twentyfouronlineConfig::get('error_font'))) {
        // Detailled error message
        $errorfont = twentyfouronlineConfig::get('error_font');
...
$timespan = read_var('timespan', $_GET, twentyfouronlineConfig::get('timespan.0.name'));
$timespan_ok = false;
foreach (twentyfouronlineConfig::get('timespan') as &$ts) {
    if ($ts['name'] == $timespan) {
        $timespan_ok = true;
    }
}
...
if ($width <= '300') {
    $rrd_cmd .= ' --font LEGEND:7:' . twentyfouronlineConfig::get('mono_font') . ' --font AXIS:6:' . twentyfouronlineConfig::get('mono_font') . ' ';
} else {
    $rrd_cmd .= ' --font LEGEND:8:' . twentyfouronlineConfig::get('mono_font') . ' --font AXIS:7:' . twentyfouronlineConfig::get('mono_font') . ' ';
}

...
The twentyfouronline dispatcher service (`twentyfouronline-service.py`) is a new method
...
The twentyfouronline service requires Python 3 and some features require
...
twentyfouronline can still use memcached as a locking mechanism when using
...
<https://docs.twentyfouronline.org/Extensions/Distributed-Poller/>

### MySQL

You should already have this, but the pollers do need access to the
SQL database. The twentyfouronline service runs faster and more aggressively
...
your twentyfouronline web UI configuration.
...
See https://your_twentyfouronline_install/poller
...
The watchdog scheduler will check that the poller log file has been written to within the last poll period. If there is no change to the log file since, the watchdog will restart the polling service. The poller log file is set by `$config['log_file']` and defaults to `./logs/twentyfouronline.log`

## Cron Scripts

Once the twentyfouronline service is installed, the cron scripts used by twentyfouronline to start alerting, polling, discovery and maintenance tasks are no longer required and must be disabled either by removing or commenting them out. The service handles these tasks when enabled.

## Service Installation

A systemd unit file is provided - You must adapt `ExecStart` and `WorkingDirectory` if you did not install twentyfouronline in `/opt/twentyfouronline`

The sysv and upstart init scripts could also be used with a little modification.

### systemd service

A systemd unit file can be found in `misc/twentyfouronline.service`. To
install run:
```bash
cp /opt/twentyfouronline/misc/twentyfouronline.service /etc/systemd/system/twentyfouronline.service && systemctl enable --now twentyfouronline.service
```
...
A systemd unit file can be found in `misc/twentyfouronline-watchdog.service`. To
install run:
```bash
cp /opt/twentyfouronline/misc/twentyfouronline-watchdog.service /etc/systemd/system/twentyfouronline.service && systemctl enable --now twentyfouronline.service
```
...
To get the twentyfouronline service running under python3.4+ on
...
# vi /opt/twentyfouronline/config.php
# vi /etc/redis.conf
# systemctl enable --now redis.service
# scl enable rh-python36 bash
# change directory to twentyfouronline (default /opt/twentyfouronline)
# pip3 install -r requirements.txt
# cp /opt/twentyfouronline/misc/twentyfouronline.service.scl /etc/systemd/system/twentyfouronline.service
# systemctl enable --now twentyfouronline.service
```
...
Warning: Bullseye provide PHP 7.4 that is too old to run twentyfouronline.

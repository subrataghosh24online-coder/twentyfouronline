...
## Add twentyfouronline user

```
useradd twentyfouronline -d /opt/twentyfouronline -M -r -s "$(which bash)"
```

## Download twentyfouronline

```
cd /opt
git clone https://github.com/twentyfouronline/twentyfouronline.git
```

## Set permissions

```
chown -R twentyfouronline:twentyfouronline /opt/twentyfouronline
chmod 771 /opt/twentyfouronline
setfacl -d -m g::rwx /opt/twentyfouronline/rrd /opt/twentyfouronline/logs /opt/twentyfouronline/bootstrap/cache/ /opt/twentyfouronline/storage/
setfacl -R -m g::rwx /opt/twentyfouronline/rrd /opt/twentyfouronline/logs /opt/twentyfouronline/bootstrap/cache/ /opt/twentyfouronline/storage/
```

## Install PHP dependencies

Change to the twentyfouronline user:
```
su - twentyfouronline
```
...
```sql
CREATE DATABASE twentyfouronline CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'twentyfouronline'@'localhost' IDENTIFIED BY 'password';
GRANT ALL PRIVILEGES ON twentyfouronline.* TO 'twentyfouronline'@'localhost';
exit
```
...
=== "Ubuntu 24.04"
    ```bash
    cp /etc/php/8.3/fpm/pool.d/www.conf /etc/php/8.3/fpm/pool.d/twentyfouronline.conf
    vi /etc/php/8.3/fpm/pool.d/twentyfouronline.conf
    ```
...
=== "CentOS 8"
    ```bash
    cp /etc/php-fpm.d/www.conf /etc/php-fpm.d/twentyfouronline.conf
    vi /etc/php-fpm.d/twentyfouronline.conf
    ```

=== "Debian 12"
    ```bash
    cp /etc/php/8.2/fpm/pool.d/www.conf /etc/php/8.2/fpm/pool.d/twentyfouronline.conf
    vi /etc/php/8.2/fpm/pool.d/twentyfouronline.conf
    ```

=== "Debian 13"
    ```bash
    cp /etc/php/8.4/fpm/pool.d/www.conf /etc/php/8.4/fpm/pool.d/twentyfouronline.conf
    vi /etc/php/8.4/fpm/pool.d/twentyfouronline.conf
    ```

Change `[www]` to `[twentyfouronline]`:
```
[twentyfouronline]
```

Change `user` and `group` to "twentyfouronline":
```
...
```
listen = /run/php-fpm-twentyfouronline.sock
```

If there are no other PHP web applications on this server, you may remove www.conf to save some resources.
Feel free to tune the performance settings in twentyfouronline.conf to meet your needs.
...
=== "Ubuntu 24.04"
    === "NGINX"
        ```bash
        vi /etc/nginx/conf.d/twentyfouronline.conf
        ```
        ...
        server {
         listen      80;
         server_name twentyfouronline.example.com;
         root        /opt/twentyfouronline/html;
         index       index.php;
         ...
         location ~ [^/]\.php(/|$) {
          fastcgi_pass unix:/run/php-fpm-twentyfouronline.sock;
...
=== "CentOS 8"
    ...
    === "Apache"
        Create the twentyfouronline.conf:

        ```
        vi /etc/httpd/conf.d/twentyfouronline.conf
        ```
        ...
        ```apache
        <VirtualHost *:80>
          DocumentRoot /opt/twentyfouronline/html/
          ServerName  twentyfouronline.example.com
          ...
          <FilesMatch ".+\.php$">
            SetHandler "proxy:unix:/run/php-fpm-twentyfouronline.sock|fcgi://localhost"
          </FilesMatch>
        </VirtualHost>
        ```
...
=== "Debian 12"
    === "NGINX"
        ```bash
        vi /etc/nginx/sites-enabled/twentyfouronline.vhost
        ```
...
=== "CentOS 8"
    ...
    <h3>Configure the contexts needed by twentyfouronline</h3>

    ```
    semanage fcontext -a -t httpd_sys_content_t '/opt/twentyfouronline/html(/.*)?'
    semanage fcontext -a -t httpd_sys_rw_content_t '/opt/twentyfouronline/(rrd|storage)(/.*)?'
    semanage fcontext -a -t httpd_log_t "/opt/twentyfouronline/logs(/.*)?"
    semanage fcontext -a -t httpd_cache_t '/opt/twentyfouronline/cache(/.*)?'
    semanage fcontext -a -t bin_t '/opt/twentyfouronline/twentyfouronline-service.py'
    semanage fcontext -a -t httpd_cache_t '/opt/twentyfouronline/cache(/.*)?'
    restorecon -RFvv /opt/twentyfouronline
    setsebool -P httpd_can_sendmail=1
    setsebool -P httpd_execmem 1
    chcon -t httpd_sys_rw_content_t /opt/twentyfouronline/.env
    ```
...
```
ln -s /opt/twentyfouronline/lnms /usr/bin/lnms
cp /opt/twentyfouronline/misc/lnms-completion.bash /etc/bash_completion.d/
```
...
```
cp /opt/twentyfouronline/snmpd.conf.example /etc/snmp/snmpd.conf
```
...
```
cp /opt/twentyfouronline/dist/twentyfouronline.cron /etc/cron.d/twentyfouronline
```
...
!!! note
    ...
    finished twentyfouronline install steps:
    <@= config.site_url =@/Support/Configuration/#proxy-support>

## Enable the scheduler

```
cp /opt/twentyfouronline/dist/twentyfouronline-scheduler.service /opt/twentyfouronline/dist/twentyfouronline-scheduler.timer /etc/systemd/system/

systemctl enable twentyfouronline-scheduler.timer
systemctl start twentyfouronline-scheduler.timer
```

## Enable logrotate

twentyfouronline keeps logs in `/opt/twentyfouronline/logs`. Over time these can
become large and be rotated out.  To rotate out the old logs you can
use the provided logrotate config file:

```
cp /opt/twentyfouronline/misc/twentyfouronline.logrotate /etc/logrotate.d/twentyfouronline
```
...
<http://twentyfouronline.example.com/install>

The web installer might prompt you to create a `config.php` file in
your twentyfouronline install location manually, copying the content displayed
...
```
chown twentyfouronline:twentyfouronline /opt/twentyfouronline/config.php
```

## Final steps

That's it!  You now should be able to log in to
<http://twentyfouronline.example.com/>.

!!! danger
    Please note that we have not covered
    HTTPS setup in this example, so your twentyfouronline install is not secure
...
<https://twentyfouronline.example.com/addhost>
...
```
sudo su - twentyfouronline
./validate.php
```

There are various options for getting help listed on the twentyfouronline web
site: <https://www.twentyfouronline.org/#support>

## What next?

Now that you've installed twentyfouronline, we'd suggest that you have a read
...
We hope you enjoy using twentyfouronline. If you do, it would be great if you
would consider opting into the stats system we have, please see [this
page](../General/Callback-Stats-and-Privacy.md) on
what it is and how to enable it.

If you would like to help make twentyfouronline better there are [many ways to
help](../Support/FAQ.md#faq9). You
can also [back twentyfouronline on Open Collective](https://t.libren.ms/donations).

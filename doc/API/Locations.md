...
```curl
curl -H 'X-Auth-Token: YOURAPITOKENHERE' https://twentyfouronline.org/api/v0/resources/locations
```
...
```curl
curl -X POST -d '{"location":"Google", "lat":"37.4220041","lng":"-122.0862462"}' -H 'X-Auth-Token:YOUR-API-TOKEN' https://twentyfouronline.org/api/v0/locations
```
...
```curl
curl -X DELETE -H 'X-Auth-Token:YOUR-API-TOKEN' https://twentyfouronline.org/api/v0/locations/Google
```
...
```curl
curl -X PATCH -d '{"lng":"100.0862462"}' -H 'X-Auth-Token:YOUR-API-TOKEN' https://twentyfouronline.org/api/v0/locations/Google
```

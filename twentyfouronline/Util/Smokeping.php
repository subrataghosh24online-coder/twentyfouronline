...
class Smokeping
{
    ...
    {
        if (is_null($this->files) && twentyfouronlineConfig::has('smokeping.dir')) {
                ...
                foreach (array_diff(scandir($dir), ['.', '..']) as $file) {
                    if (stripos($file, '.rrd') !== false) {
                        if (strpos($file, '~') !== false) {
                            ...
                            $this->files['in'][$target][twentyfouronlineConfig::get('own_hostname')] = $file;
                            $this->files['out'][twentyfouronlineConfig::get('own_hostname')][$target] = $file;
                        }
                    }
                }
            }
    ...
    {
        if (twentyfouronlineConfig::get('smokeping.integration') === true) {
            return twentyfouronlineConfig::get('smokeping.dir') . '/' . ($this->device->type ?: 'Ungrouped') . '/' . $file;
        } else {
            return twentyfouronlineConfig::get('smokeping.dir') . '/' . $file;
        }
    }
        ...
        foreach ($this->getFiles()[$direction][$this->device->hostname] as $remote_host => $file) {
            if (Str::contains($file, '~')) {
                ...
                if (empty($device->device_id)) {
                    \Log::debug('Could not find smokeping slave device in twentyfouronline', ['slave' => $remote_host]);
                    continue;
                }

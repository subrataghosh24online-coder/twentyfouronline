...
class Mail
{
        ...
        if (is_string($emails)) {
            ...
            foreach ($emails as $email) {
                if (preg_match($regex, $email, $out, PREG_OFFSET_CAPTURE)) {
                    ...
                    if (strpos($email, '@')) {
                        $from_name = twentyfouronlineConfig::get('email_user');
    ...
    public static function send($emails, $subject, $message, bool $html = false, bool $bcc = false, ?bool $embedGraphs = null): bool
    {
        if (is_array($emails) || ($emails = self::parseEmails($emails))) {
            ...
            foreach (self::parseEmails(twentyfouronlineConfig::get('email_from')) as $from => $from_name) {
            ...
            $mail->XMailer = twentyfouronlineConfig::get('project_name');
            $mail->CharSet = 'utf-8';
            $mail->WordWrap = 76;
            $mail->Body = $message;
            if ($embedGraphs ?? twentyfouronlineConfig::get('email_attach_graphs')) {
                self::embedGraphs($mail, $html);
            }
            if ($html) {
                $mail->isHTML();
            }
            switch (strtolower(trim(twentyfouronlineConfig::get('email_backend')))) {
                case 'sendmail':
                    $mail->Mailer = 'sendmail';
                    $mail->Sendmail = twentyfouronlineConfig::get('email_sendmail_path');
                    break;
                case 'smtp':
                    $mail->isSMTP();
                    $mail->Host = twentyfouronlineConfig::get('email_smtp_host');
                    $mail->Timeout = twentyfouronlineConfig::get('email_smtp_timeout');
                    $mail->SMTPAuth = twentyfouronlineConfig::get('email_smtp_auth');
                    $mail->SMTPSecure = twentyfouronlineConfig::get('email_smtp_secure');
                    $mail->Port = twentyfouronlineConfig::get('email_smtp_port');
                    $mail->Username = twentyfouronlineConfig::get('email_smtp_username');
                    $mail->Password = twentyfouronlineConfig::get('email_smtp_password');
                    $mail->SMTPAutoTLS = twentyfouronlineConfig::get('email_auto_tls');
                    $mail->SMTPDebug = 0;
                    $mail->SMTPOptions = [
                        'ssl' => [
                            'verify_peer' => twentyfouronlineConfig::get('email_smtp_verifypeer', true),
                            'allow_self_signed' => twentyfouronlineConfig::get('email_smtp_allowselfsigned', false),
                        ],
                    ];
                    break;
                default:
    ...
    private static function embedGraphs(PHPMailer $mail, bool $html = false): void
    {
        ...
        preg_match_all('#<img class=\"twentyfouronline-graph\" src=\"(.*?)\" ?/?>#', $body, $matches);

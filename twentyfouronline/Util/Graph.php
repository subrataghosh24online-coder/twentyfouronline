...
use twentyfouronline\Data\Graphing\GraphImage;
...
class Graph
{
    ...
    public static function getSubtypes($type, $device = null): array
    {
        ...
        if ($device != null) {
            ...
            foreach (twentyfouronlineConfig::get('graph_types') as $type => $type_data) {
    ...
    {
        return twentyfouronlineConfig::get("graph_types.$type.$subtype.section") == 'mib';
    }

    public static function getOverviewGraphsForDevice(Device $device): array
    {
        if ($device->snmp_disable) {
            return Arr::wrap(twentyfouronlineConfig::getOsSetting('ping', 'over'));
        }

        if ($graphs = twentyfouronlineConfig::getOsSetting($device->os, 'over')) {
            return Arr::wrap($graphs);
        }

        $os_group = twentyfouronlineConfig::getOsSetting($device->os, 'group');

        return Arr::wrap(twentyfouronlineConfig::get("os_group.$os_group.over", twentyfouronlineConfig::get('os.default.over')));
    }
    ...
    {
        $type = twentyfouronlineConfig::get('webui.graph_type');

...
class ModuleTestHelper
{
    ...
    public function __construct($modules, $os, $variant = '')
    {
        ...
        $install_dir = twentyfouronlineConfig::get('install_dir');
        ...
        twentyfouronlineConfig::set('hide_rrd_disabled', true);
    ...
    public static function findOsWithData($modules = [], ?string $os_filter = null)
    {
        ...
        foreach (glob(twentyfouronlineConfig::get('install_dir') . '/tests/data/*.json') as $file) {
    ...
    {
        global $device;
        twentyfouronlineConfig::set('rrd.enable', false); // disable rrd
        twentyfouronlineConfig::set('rrdtool_version', '1.7.2'); // don't detect rrdtool version, rrdtool is not install on ci
    ...
    {
        if ($short) {
            return ltrim(str_replace(twentyfouronlineConfig::get('install_dir'), '', $this->json_file), '/');
        }

...
namespace twentyfouronline\OS\Traits;
...
trait VminfoLibvirt
{
    public function discoverVminfo(): Collection
    {
        ...
        if (! twentyfouronlineConfig::get('enable_libvirt')) {
        ...
        if (twentyfouronlineConfig::has('libvirt_username')) {
            $userHostname = twentyfouronlineConfig::get('libvirt_username') . '@' . $userHostname;
        }

        foreach (twentyfouronlineConfig::get('libvirt_protocols') as $method) {
            ...
            if ($ssh_ok || ! Str::contains($method, 'ssh')) {
                ...
                unset($domlist);
                exec(twentyfouronlineConfig::get('virsh') . ' -rc ' . $uri . ' list', $domlist);
                ...
                foreach ($domlist as $dom) {
                    ...
                    if (is_numeric($dom_id)) {
                        ...
                        unset($vm_info_array);
                        exec(twentyfouronlineConfig::get('virsh') . ' -rc ' . $uri . ' dumpxml ' . $dom_id, $vm_info_array);
                        ...
                        exec(twentyfouronlineConfig::get('virsh') . ' -rc ' . $uri . ' domstate ' . $dom_id, $vm_state);

...
namespace twentyfouronline\Snmptrap;
...
class Dispatcher
{
    ...
    public static function handle(Trap $trap): bool
    {
        ...
        if ($trap->findOid('iso.3.6.1.6.3.1.1.4.1.0')) {
            ...
            // twentyfouronline snmptraps code cannot process received data. Let's inform the user.
            Eventlog::log('Misconfigured MIBS or MIBDIRS for snmptrapd, check https://docs.twentyfouronline.org/Extensions/SNMP-Trap-Handler/ : ' . $trap->raw, $trap->getDevice(), 'system');

            return false;
        }

        // note, this doesn't clear the resolved SnpmtrapHandler so only one per run
        /** @var \twentyfouronline\Interfaces\SnmptrapHandler $handler */
        $handler = app(\twentyfouronline\Interfaces\SnmptrapHandler::class, [$trap->getTrapOid()]);
        $handler->handle($trap->getDevice(), $trap);

        // log an event if appropriate
        $fallback = $handler instanceof Fallback;
        $logging = twentyfouronlineConfig::get('snmptraps.eventlog', 'unhandled');
        $detailed = twentyfouronlineConfig::get('snmptraps.eventlog_detailed', false);

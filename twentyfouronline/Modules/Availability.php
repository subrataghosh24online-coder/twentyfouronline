...
class Availability implements Module
{
    ...
    public function poll(OS $os, DataStorageInterface $datastore): void
    {
        ...
        $valid_ids = [];
        foreach (twentyfouronlineConfig::get('graphing.availability') as $duration) {
            // update database with current calculation
            $avail = \App\Models\Availability::updateOrCreate([
                'device_id' => $os->getDeviceId(),
                'duration' => $duration,
            ], [
                'availability_perc' => \twentyfouronline\Device\Availability::availability($os->getDevice(), $duration),
            ]);

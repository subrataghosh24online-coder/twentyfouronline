...
use twentyfouronline\Util\Compare;
...
class Core implements Module
{
    ...
    public function discover(OS $os): void
    {
        ...
        $loaded_os_type = twentyfouronlineConfig::get("os.$device->os.type");
        ...
        Log::notice('OS: ' . twentyfouronlineConfig::getOsSetting($device->os, 'text') . " ($device->os)\n");
    }
    ...
    public static function detectOS(Device $device, bool $fetch = true): string
    {
        ...
        // check yaml files
        $os_defs = twentyfouronlineConfig::get('os');
    ...
    private function calculateUptime(OS $os, ?string $sysUpTime, DataStorageInterface $datastore): void
    {
        ...
        if (twentyfouronlineConfig::get("os.$device->os.bad_uptime")) {
            return;
        }
        ...
        if (! empty($agent_data['uptime'])) {
            ...
            $uptime = max(
                ...
                twentyfouronlineConfig::get("os.$device->os.bad_snmpEngineTime") ? 0 : Number::cast($uptime_data['SNMP-FRAMEWORK-MIB::snmpEngineTime.0'] ?? 0),
                twentyfouronlineConfig::get("os.$device->os.bad_hrSystemUptime") ? 0 : round(Number::cast($uptime_data['HOST-RESOURCES-MIB::hrSystemUptime.0'] ?? 0) / 100)
            );
        ...
        if ($uptime > 0) {
            if ($uptime < $device->uptime) {
                ...
                if (twentyfouronlineConfig::get('discovery_on_reboot')) {

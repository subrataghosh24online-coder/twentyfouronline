...
class SSOAuthorizer extends MysqlAuthorizer
{
    ...
    public function authenticate($credentials)
    {
        ...
        if (twentyfouronlineConfig::get('sso.create_users') || twentyfouronlineConfig::get('sso.update_users')) {
            $user = User::thisAuth()->firstOrNew(['username' => $credentials['username']]);

            $create = ! $user->exists && twentyfouronlineConfig::get('sso.create_users');
            $update = $user->exists && twentyfouronlineConfig::get('sso.update_users');

            if ($create || $update) {
                $user->auth_type = LegacyAuth::getType();
                $user->can_modify_passwd = 0;
                $user->realname = $this->authSSOGetAttr(twentyfouronlineConfig::get('sso.realname_attr'));
                $user->email = $this->authSSOGetAttr(twentyfouronlineConfig::get('sso.email_attr'));
                $user->descr = $this->authSSOGetAttr(twentyfouronlineConfig::get('sso.descr_attr')) ?: 'SSO User';
                $user->save();
            }
        }
    ...
    {
        return $this->authSSOGetAttr(twentyfouronlineConfig::get('sso.user_attr'), '');
    }
    ...
    public function authSSOGetAttr($attr, $prefix = 'HTTP_')
    {
        ...
        if ($this->authSSOProxyTrusted()) {
            ...
            if (twentyfouronlineConfig::get('sso.mode') === 'header' && array_key_exists($header_key, $_SERVER)) {
                return $_SERVER[$header_key];
            } elseif (twentyfouronlineConfig::get('sso.mode') === 'env' && array_key_exists($attr, $_SERVER)) {
                return $_SERVER[$attr];
            } else {
                return null;
            }
    ...
    public function authSSOProxyTrusted()
    {
        ...
        if (twentyfouronlineConfig::get('sso.trusted_proxies')) {
            try {
                ...
                $proxies = twentyfouronlineConfig::get('sso.trusted_proxies');
    ...
    {
        if (twentyfouronlineConfig::get('sso.group_strategy') === 'attribute') {
            if (twentyfouronlineConfig::get('sso.level_attr')) {
                if (is_numeric($this->authSSOGetAttr(twentyfouronlineConfig::get('sso.level_attr')))) {
                    return Arr::wrap(LegacyAuthLevel::tryFrom((int) $this->authSSOGetAttr(twentyfouronlineConfig::get('sso.level_attr')))?->getName());
                } else {
        ...
        } elseif (twentyfouronlineConfig::get('sso.group_strategy') === 'map') {
            if (twentyfouronlineConfig::get('sso.group_level_map') && is_array(twentyfouronlineConfig::get('sso.group_level_map')) && twentyfouronlineConfig::get('sso.group_delimiter') && twentyfouronlineConfig::get('sso.group_attr')) {
        ...
        } elseif (twentyfouronlineConfig::get('sso.group_strategy') === 'static') {
            if (twentyfouronlineConfig::get('sso.static_level')) {
                return Arr::wrap(LegacyAuthLevel::tryFrom((int) twentyfouronlineConfig::get('sso.static_level'))?->getName());
            } else {
    ...
    public function authSSOParseGroups()
    {
        ...
        $groups = explode(twentyfouronlineConfig::get('sso.group_delimiter', ';'), $this->authSSOGetAttr(twentyfouronlineConfig::get('sso.group_attr')) ?? '');

        $valid_groups = [];

        // Only consider groups that match the filter expression - this is an optimisation for sites with thousands of groups
        if (twentyfouronlineConfig::get('sso.group_filter')) {
            foreach ($groups as $group) {
                if (preg_match(twentyfouronlineConfig::get('sso.group_filter'), $group)) {
        ...
        $level = (int) twentyfouronlineConfig::get('sso.static_level', 0);

        $config_map = twentyfouronlineConfig::get('sso.group_level_map');

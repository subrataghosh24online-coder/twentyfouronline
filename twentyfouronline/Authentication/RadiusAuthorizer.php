...
class RadiusAuthorizer extends MysqlAuthorizer
{
    ...
    {
        $this->radius = new Radius(twentyfouronlineConfig::get('radius.hostname'), twentyfouronlineConfig::get('radius.secret'), twentyfouronlineConfig::get('radius.suffix'), twentyfouronlineConfig::get('radius.timeout'), twentyfouronlineConfig::get('radius.port'));
    }
    ...
    public function authenticate($credentials)
    {
        ...
        if ($this->radius->accessRequest($credentials['username'], $password) === true) {
            ...
            if ($filter_id_attribute && Str::startsWith($filter_id_attribute, 'twentyfouronline_role_')) {
                $this->roles[$credentials['username']] = [substr($filter_id_attribute, 14)];
            }

            if (twentyfouronlineConfig::get('radius.enforce_roles') || $new_user) {
    ...
    private function getDefaultRoles(): array
    {
        ...
        return twentyfouronlineConfig::get('radius.default_roles')
            ?: Arr::wrap(LegacyAuthLevel::from(twentyfouronlineConfig::get('radius.default_level') ?? 1)->getName());
    }
}

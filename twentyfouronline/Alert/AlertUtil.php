...
class AlertUtil
{
    ...
    public static function getContacts($results)
    {
        ...
        if (twentyfouronlineConfig::get('alert.default_only') === true || twentyfouronlineConfig::get('alerts.email.default_only') === true) {
            $email = twentyfouronlineConfig::get('alert.default_mail', twentyfouronlineConfig::get('alerts.email.default'));
        ...
        if (twentyfouronlineConfig::get('alert.syscontact')) {
            $contacts = array_merge($contacts, self::findContactsSysContact($results));
        }

        if (twentyfouronlineConfig::get('alert.users')) {
            $contacts = array_merge($contacts, self::findContactsOwners($results));
        }

        $roles = twentyfouronlineConfig::get('alert.globals')
            ? ['admin', 'global-read']
            : (twentyfouronlineConfig::get('alert.admins') ? ['admin'] : []);
        if ($roles) {
        ...
        $default_mail = twentyfouronlineConfig::get('alert.default_mail');
        if (! isset($tmp_contacts[$default_mail]) && twentyfouronlineConfig::get('alert.default_copy')) {
            $tmp_contacts[$default_mail] = '';
        }
        // Send email to default contact if no other contact found
        if (empty($tmp_contacts) && twentyfouronlineConfig::get('alert.default_if_none') && $default_mail) {
            $tmp_contacts[$default_mail] = '';
        }
    ...
    {
        $macros = twentyfouronlineConfig::get('alert.macros.rule', []);
        krsort($macros);

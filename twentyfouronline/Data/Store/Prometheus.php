...
class Prometheus extends BaseDatastore
{
    ...
    public function __construct()
    {
        ...
        $url = twentyfouronlineConfig::get('prometheus.url');
        $job = twentyfouronlineConfig::get('prometheus.job', 'twentyfouronline');
        $this->base_uri = "$url/metrics/job/$job/instance/";

        $this->client = Http::client()->baseUrl($this->base_uri);

        $user = twentyfouronlineConfig::get('prometheus.user', '');
        $passwd = twentyfouronlineConfig::get('prometheus.password', '');
        if ($user && $passwd) {
            $this->client = $this->client->withBasicAuth($user, $passwd);
        }

        $this->prefix = twentyfouronlineConfig::get('prometheus.prefix', '');
        if ($this->prefix) {
    ...
    {
        return twentyfouronlineConfig::get('prometheus.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        ...
        if (twentyfouronlineConfig::get('prometheus.attach_sysname', false)) {

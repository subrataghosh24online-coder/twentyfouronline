...
class InfluxDBv2 extends BaseDatastore
{
    ...
    public function __construct()
    {
        ...
        try {
            $host = twentyfouronlineConfig::get('influxdbv2.host', 'localhost');
            $transport = twentyfouronlineConfig::get('influxdbv2.transport', 'http');
            $port = twentyfouronlineConfig::get('influxdbv2.port', 8086);
            $bucket = twentyfouronlineConfig::get('influxdbv2.bucket', 'twentyfouronline');
            $organization = twentyfouronlineConfig::get('influxdbv2.organization', '');
            $allow_redirects = twentyfouronlineConfig::get('influxdbv2.allow_redirects', true);
            $token = twentyfouronlineConfig::get('influxdbv2.token', '');
            $debug = twentyfouronlineConfig::get('influxdbv2.debug', false);
            $log_file = twentyfouronlineConfig::get('influxdbv2.log_file', twentyfouronlineConfig::get('log_file'));
            $timeout = twentyfouronlineConfig::get('influxdbv2.timeout', 1);
            $verify = twentyfouronlineConfig::get('influxdbv2.verify', false);
            $batch_size = twentyfouronlineConfig::get('influxdbv2.batch_size', 1000);
            $max_retry = twentyfouronlineConfig::get('influxdbv2.max_retry', 3);
    ...
    {
        return twentyfouronlineConfig::get('influxdbv2.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        ...
        $excluded_groups = twentyfouronlineConfig::get('influxdbv2.groups-exclude');
        ...
        if (twentyfouronlineConfig::get('influxdbv2.debug') === true) {

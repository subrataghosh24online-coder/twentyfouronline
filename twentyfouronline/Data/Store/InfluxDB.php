...
class InfluxDB extends BaseDatastore
{
    ...
    public function __construct(Database $influx)
    {
        ...
        $this->batchSize = twentyfouronlineConfig::get('influxdb.batch_size', 0);
        $this->measurements = twentyfouronlineConfig::get('influxdb.measurements', []);
    ...
    {
        return twentyfouronlineConfig::get('influxdb.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        ...
        if (twentyfouronlineConfig::get('influxdb.debug', false) === true) {
    ...
    {
        $host = twentyfouronlineConfig::get('influxdb.host', 'localhost');
        $transport = twentyfouronlineConfig::get('influxdb.transport', 'http');
        $port = twentyfouronlineConfig::get('influxdb.port', 8086);
        $db = twentyfouronlineConfig::get('influxdb.db', 'twentyfouronline');
        $username = twentyfouronlineConfig::get('influxdb.username', '');
        $password = twentyfouronlineConfig::get('influxdb.password', '');
        $timeout = twentyfouronlineConfig::get('influxdb.timeout', 0);
        $verify_ssl = twentyfouronlineConfig::get('influxdb.verifySSL', false);

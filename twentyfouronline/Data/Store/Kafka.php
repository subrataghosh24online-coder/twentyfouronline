...
namespace twentyfouronline\Data\Store;
...
class Kafka extends BaseDatastore
{
    ...
    public function __construct(Producer $client)
    {
        ...
        $this->topicName = twentyfouronlineConfig::get('kafka.topic', 'twentyfouronline');

        // Cache the flush timeout value early to avoid Config during shutdown
        if ($this->kafkaFlushTimeout == null) {
            $this->kafkaFlushTimeout = twentyfouronlineConfig::get('kafka.flush.timeout', 100);
        }

        // Load excluded values from config
        foreach (twentyfouronlineConfig::get('kafka.groups-exclude', []) as $exclude_group) {
        ...
        foreach (twentyfouronlineConfig::get('kafka.measurement-exclude', []) as $exclude_measurement) {
    ...
    public static function getClient(): Producer
    {
        ...
        $conf->set('bootstrap.servers', twentyfouronlineConfig::get('kafka.broker.list', '127.0.2.2:9092'));
        // Set the idempotence
        $conf->set('enable.idempotence', twentyfouronlineConfig::get('kafka.idempotence', false) ? 'true' : 'false');
        // Max queue allowed messages in poller memory
        $conf->set('queue.buffering.max.messages', twentyfouronlineConfig::get('kafka.buffer.max.message', 1_000));
        // Num of messages each call to kafka
        $conf->set('batch.num.messages', twentyfouronlineConfig::get('kafka.batch.max.message', 200));
        // Max wait time to acumulate before sending the batch
        $conf->set('linger.ms', twentyfouronlineConfig::get('kafka.linger.ms', default: 50));
        // Change ACK
        $conf->set(
            ...
            twentyfouronlineConfig::get('kafka.idempotence', false) ? 'all' :
                (twentyfouronlineConfig::get('kafka.request.required.acks', '-1'))
        );

        // check if debug for ssl was set and enable it
        $confKafkaSSLDebug = twentyfouronlineConfig::get('kafka.security.debug', null);
        $confKafkaSSLDebug != null || strlen($confKafkaSSLDebug) !== 0 ? $conf->set('debug', $confKafkaSSLDebug) : null;

        // config ssl
        $isSslEnabled = twentyfouronlineConfig::get('kafka.ssl.enable', false);
        if ($isSslEnabled) {
            $conf->set('security.protocol', twentyfouronlineConfig::get('kafka.ssl.protocol', 'ssl'));
            $conf->set('ssl.endpoint.identification.algorithm', 'none');

            // prepare all necessary twentyfouronline kafka config with associated rdkafka key
            $kafkaSSLConfigs = [
            ...
            foreach ($kafkaSSLConfigs as $configKey => $kafkaKey) {
                $configValue = twentyfouronlineConfig::get($configKey, null);
    ...
    {
        return twentyfouronlineConfig::get('kafka.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        try {
            ...
            if (twentyfouronlineConfig::get('kafka.debug') === true) {

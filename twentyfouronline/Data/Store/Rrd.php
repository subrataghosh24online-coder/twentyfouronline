...
use twentyfouronline\Exceptions\FileExistsException;
use twentyfouronline\Exceptions\RrdException;
use twentyfouronline\Exceptions\RrdGraphException;
use twentyfouronline\Proc;
...
class Rrd extends BaseDatastore
{
    ...
    {
        return twentyfouronlineConfig::get('rrd.enable', true);
    }

    protected function loadConfig(): void
    {
        $this->rrdcached = twentyfouronlineConfig::get('rrdcached', false);
        $this->rrd_dir = twentyfouronlineConfig::get('rrd_dir', twentyfouronlineConfig::get('install_dir') . '/rrd');
        $this->step = twentyfouronlineConfig::get('rrd.step', 300);
        $this->rra = twentyfouronlineConfig::get(
            'rrd_rra',
        ...
        $this->version = twentyfouronlineConfig::get('rrdtool_version', '1.4');
        $this->rrdtool_executable = twentyfouronlineConfig::get('rrdtool', 'rrdtool');
    }
    ...
    /**
     * rename an rrdfile, can only be done on the twentyfouronline server hosting the rrd files
     *
    ...
    private function command($command, $filename, $options): array
    {
        ...
        if ($this->disabled && ! in_array($command, $ro_commands)) {
            if (! twentyfouronlineConfig::get('hide_rrd_disabled')) {
    ...
    {
        $proc = new Process([twentyfouronlineConfig::get('rrdtool', 'rrdtool'), '--version']);
        $proc->run();

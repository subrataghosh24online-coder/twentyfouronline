...
use twentyfouronline\Exceptions\FpingUnparsableLine;
use Log;
...
class Fping
{
    ...
    public function __construct()
    {
        ...
        $this->fping_bin = twentyfouronlineConfig::get('fping', 'fping');
        $fping6 = twentyfouronlineConfig::get('fping6', 'fping6');
        $this->fping6_bin = is_executable($fping6) ? $fping6 : false;
        $this->count = max(twentyfouronlineConfig::get('fping_options.count', 3), 1);
        $this->interval = max(twentyfouronlineConfig::get('fping_options.interval', 500), 20);
        $this->timeout = max(twentyfouronlineConfig::get('fping_options.timeout', 500), $this->interval);
        $this->retries = twentyfouronlineConfig::get('fping_options.retries', 2);
        $this->tos = twentyfouronlineConfig::get('fping_options.tos', 0);
    }
    ...
    public function bulkPing(array $hosts, callable $callback): void
    {
        ...
        $process->setTimeout(twentyfouronlineConfig::get('rrd.step', 300) * 2);

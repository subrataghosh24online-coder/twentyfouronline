...
class QueueManager:
    def __init__(
        ...
        :param config: twentyfouronline.ServiceConfig reference to the service config object
        :param lock_manager: A twentyfouronline.Lock instance to help with locks
        ...
        self.config = config
        self.performance = twentyfouronline.PerformanceCounter()
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if (
                    ...
                    with twentyfouronline.TimeitContext.start() as t:
    ...
    def get_poller_config(self):
        """
        Returns the twentyfouronline.PollerConfig for this QueueManager
        :return: twentyfouronline.PollerConfig
        """
    ...
    def _create_queue(self, queue_type, group):
        ...
        try:
            return twentyfouronline.RedisUniqueQueue(
                ...
                namespace="twentyfouronline.queue",
        ...
        return twentyfouronline.UniqueQueue()
...
class TimedQueueManager(QueueManager):
    def __init__(
        ...
        self.timer = twentyfouronline.RecurringTimer(
...
class BillingQueueManager(TimedQueueManager):
    def __init__(self, config, lock_manager):
        ...
        self.calculate_timer = twentyfouronline.RecurringTimer(
    ...
    def do_work(self, run_type, group):
        if run_type == "poll":
            ...
            exit_code, output = twentyfouronline.call_script("poll-billing.php", args)
        else:  # run_type == 'calculate'
            logger.info("Calculating billing")
            args = ("-d", "-f") if self.config.debug else ("-f")
            exit_code, output = twentyfouronline.call_script("billing-calculate.php", args)
    ...
    def do_work(self, context, group):
        if self.lock(group, "group", timeout=self.config.ping.frequency):
            try:
                ...
                exit_code, output = twentyfouronline.call_script("ping.php", args)
...
class ServicesQueueManager(TimedQueueManager):
    ...
    def do_work(self, device_id, group):
        if self.lock(device_id, timeout=self.config.services.frequency):
            ...
            exit_code, output = twentyfouronline.call_script("check-services.php", args)
...
class AlertQueueManager(TimedQueueManager):
    ...
    def do_work(self, device_id, group):
        ...
        args = ("-d", "-f") if self.config.debug else ("-f",)
        exit_code, output = twentyfouronline.call_script("alerts.php", args)
...
class PollerQueueManager(QueueManager):
    ...
    def do_work(self, device_id, group):
        if self.lock(device_id, timeout=self.config.poller.frequency):
            ...
            exit_code, output = twentyfouronline.call_script("lnms", args)
...
class DiscoveryQueueManager(TimedQueueManager):
    ...
    def do_work(self, device_id, group):
        if self.lock(
            device_id, timeout=twentyfouronline.normalize_wait(self.config.discovery.frequency)
        ):
            logger.info("Discovering device {}".format(device_id))

            args = ("-d", "-h", device_id) if self.config.debug else ("-h", device_id)
            exit_code, output = twentyfouronline.call_script("discovery.php", args)

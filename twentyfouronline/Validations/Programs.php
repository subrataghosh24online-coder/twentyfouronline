...
class Programs extends BaseValidation
{
    ...
    {
        $validator->info('fping FAILURES can be ignored if running twentyfouronline in a jail without ::1. You may want to test it manually: fping ::1');
        $validator->fail(
    ...
    {
        if (is_executable(twentyfouronlineConfig::get($bin))) {
            return twentyfouronlineConfig::get($bin);
        }

        $located = twentyfouronlineConfig::locateBinary($bin);

...
class Python extends BaseValidation
{
    ...
    private function checkExtensions(Validator $validator)
    {
        ...
        $process = new Process([twentyfouronlineConfig::get('install_dir') . $pythonExtensions, '-v']);
        $process->run();

        if ($process->getExitCode() !== 0) {
            $user = \config('twentyfouronline.user');
            ...
            if ($user_mismatch) {
                $validator->warn(
                    ...
                    'The install docs show how this is done on a new install: https://docs.twentyfouronline.org/Installation/Install-twentyfouronline/#configure-php-fpm'
                );
            } else {
                $validator->fail("Python3 module issue found: '" . $process->getOutput() . "'", 'pip3 install -r ' . twentyfouronlineConfig::get('install_dir') . '/requirements.txt');
            }
        }
    }
}

...
from twentyfouronline.config import DBConfig
...
class ServiceConfig(DBConfig):
    def __init__(self):
        """
        Stores all of the configuration variables for the twentyfouronline service in a common object
    ...
    watchdog_logfile = "logs/twentyfouronline.log"
    health_file = ""  # disabled by default

    def populate(self):
        config = twentyfouronline.get_config_data(self.BASE_DIR)
        ...
        self.watchdog_logfile = config.get("log_file", self.logdir + "/twentyfouronline.log")
...
class Service:
    ...
    def __init__(self):
        ...
        self._db = twentyfouronline.DB(self.config)
        ...
        self.daily_timer = twentyfouronline.RecurringTimer(
            self.config.update_frequency, self.run_maintenance, "maintenance"
        )
        self.stats_timer = twentyfouronline.RecurringTimer(
        ...
        if self.config.watchdog_enabled:
            ...
            self.watchdog_timer = twentyfouronline.RecurringTimer(
        ...
        self.systemd_watchdog_timer = twentyfouronline.RecurringTimer(
    ...
    def start(self):
        ...
        self.poller_manager = twentyfouronline.PollerQueueManager(self.config, self._lm)
        self.queue_managers["poller"] = self.poller_manager
        self.discovery_manager = twentyfouronline.DiscoveryQueueManager(self.config, self._lm)
        self.queue_managers["discovery"] = self.discovery_manager
        self.queue_managers["alerting"] = twentyfouronline.AlertQueueManager(
            self.config, self._lm
        )
        self.queue_managers["services"] = twentyfouronline.ServicesQueueManager(
            self.config, self._lm
        )
        self.queue_managers["billing"] = twentyfouronline.BillingQueueManager(
            self.config, self._lm
        )
        self.queue_managers["ping"] = twentyfouronline.PingQueueManager(self.config, self._lm)
        ...
        logger.info("twentyfouronline Service: {} started!".format(self.config.unique_name))
        logger.info(
            "Poller group {}. Using Python {} and {} locks and queues".format(
                "0 (default)" if self.config.group == [0] else self.config.group,
                python_version(),
                "redis" if isinstance(self._lm, twentyfouronline.RedisLock) else "internal",
            )
        )
        logger.info(
    ...
    def run_maintenance(self):
        ...
        exit_code, output = twentyfouronline.call_script("daily.sh")
        if exit_code == 0:
    ...
    def create_lock_manager(self):
        ...
        """
        try:
            return twentyfouronline.RedisLock(
                sentinel_kwargs={
                ...
                namespace="twentyfouronline.lock",
        ...
        return twentyfouronline.ThreadingLock()

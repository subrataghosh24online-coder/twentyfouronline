...
namespace twentyfouronline\Device;
...
use twentyfouronline\Interfaces\Discovery\DiscoveryItem;
...
class YamlDiscovery
{
     ...
     * @return string
     *
     * @throws \twentyfouronline\Exceptions\InvalidOidException
     */
        ...
        if (! empty($os->getDiscovery()['modules'])) {
            ...
            foreach ($os->getDiscovery()['modules'] as $module => $discovery_data) {
                ...
                foreach ($discovery_data as $key => $data_array) {
                    ...
                    $saved_nobulk = twentyfouronlineConfig::getOsSetting($os->getName(), 'snmp_bulk', true);
                        ...
                        foreach (Arr::wrap($data['oid'] ?? []) as $oid) {
                            if (! array_key_exists($oid, $pre_cache)) {
                                ...
                                if (isset($data['snmp_bulk'])) {
                                    twentyfouronlineConfig::set('os.' . $os->getName() . '.snmp_bulk', (bool) $data['snmp_bulk']);
                                }

                                $mib = $os->getDiscovery()['mib'] ?? null;
                                $pre_cache[$oid] = snmpwalk_cache_oid($device, $oid, $pre_cache[$oid] ?? [], $mib, null, $snmp_flag);

                                twentyfouronlineConfig::set('os.' . $os->getName() . '.snmp_bulk', $saved_nobulk);
                            }
                        }
                    }
                }
